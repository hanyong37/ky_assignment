  ...
  def generate_invoices
    self.rent_phases.each do |rp|
      rp.get_bill_list.each do |bl|
        #创建invoice；
        #如果账单周期存在，则合并；
        if Invoice.where(contract_id: self.id, due_date: bl[:due_date]).exists?
            old_inv =Invoice.where(contract_id: self.id, due_date: bl[:due_date]).first
